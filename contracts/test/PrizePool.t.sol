...
contract PrizePoolTest is Test {
    ...
    function _setupCompleteHackathon() internal returns (uint256 hackathonId) {
        ...
        // Submit projects
        vm.prank(participant1);
        registry.updateSubmission(
            hackathonId,
            "https://github.com/participant1/project"
        );

        vm.prank(participant2);
        registry.updateSubmission(
            hackathonId,
            "https://github.com/participant2/project"
        );

        vm.prank(participant3);
        registry.updateSubmission(
            hackathonId,
            "https://github.com/participant3/project"
        );
    ...
    function test_CreatePrizePool() public {
        ...
        assertEq(
            totalPrizePool,
            PRIZE_AMOUNT,
            "Prize pool should match sent amount"
        );
    ...
    function test_SetPrizeDistribution() public {
        ...
        (uint16[] memory percentages, , , , , ) = prizePool
            .getPrizeConfiguration(hackathonId);
    ...
    function test_DistributePrizes_WinnerTakesAll() public {
        ...
        // Check distribution
        assertTrue(
            prizePool.isPrizeDistributed(hackathonId),
            "Prizes should be distributed"
        );

        // Check winner payout (only 1st place gets everything)
        (address winner, uint256 amount, bool paid) = prizePool.getWinnerPayout(
            hackathonId,
            1
        );
    ...
    function test_DistributePrizes_MultipleWinners() public {
        ...
        assertEq(
            amount1,
            (PRIZE_AMOUNT * 6000) / 10000,
            "1st place should get 60%"
        );
        ...
        assertEq(
            amount2,
            (PRIZE_AMOUNT * 2500) / 10000,
            "2nd place should get 25%"
        );
        ...
        assertEq(
            amount3,
            (PRIZE_AMOUNT * 1500) / 10000,
            "3rd place should get 15%"
        );
    ...
    function test_WithdrawPrize() public {
        ...
        assertEq(
            ...
            PRIZE_AMOUNT,
            "Should receive full prize amount"
        );
    ...
    function test_WithdrawAllPrizes() public {
        ...
        assertEq(
            balanceAfter - balanceBefore,
            expectedAmount,
            "Should receive 1st place prize"
        );
    }
    ...
    function test_EmergencyWithdraw() public {
        ...
        assertEq(
            prizePool.getTotalPrizePool(hackathonId),
            0,
