...
contract SetupIntegration is Script {
    ...
    function setUp() public {
        ...
        hackathonRegistry = HackathonRegistry(
            vm.envAddress("HACKATHON_REGISTRY_ADDRESS")
        );
    ...
    function _createPrizePool() internal {
        ...
        require(
            organizer.balance >= PRIZE_AMOUNT,
            "Insufficient balance for prize pool"
        );
    ...
    function _submitProjects() internal {
        ...
        vm.broadcast(participant1);
        hackathonRegistry.updateSubmission(
            hackathonId,
            "https://github.com/participant1/amazing-project"
        );

        vm.broadcast(participant2);
        hackathonRegistry.updateSubmission(
            hackathonId,
            "https://github.com/participant2/great-dapp"
        );

        vm.broadcast(participant3);
        hackathonRegistry.updateSubmission(
            hackathonId,
            "https://github.com/participant3/cool-protocol"
        );
    ...
    function _conductVoting() internal {
        ...
        // Show results
        (address first, address second, address third) = hackathonRegistry
    ...
    function _distributePrizes() internal {
        ...
        // Show prize amounts
        (address winner1, uint256 amount1, ) = prizePool.getWinnerPayout(
            hackathonId,
            1
        );
        (address winner2, uint256 amount2, ) = prizePool.getWinnerPayout(
            hackathonId,
            2
        );
        (address winner3, uint256 amount3, ) = prizePool.getWinnerPayout(
            hackathonId,
            3
        );

        console.log("Prize Amounts:");
        console.log("  1st:", winner1);
        console.log("  1st amount:", amount1 / 1e18);
        console.log("  2nd:", winner2);
        console.log("  2nd amount:", amount2 / 1e18);
        console.log("  3rd:", winner3);
        console.log("  3rd amount:", amount3 / 1e18);
    }
