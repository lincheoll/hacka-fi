...
import { getStatusSummary, triggerStatusCheck, getAuditStatistics } from '@/lib/api-functions';
...
export function StatusDashboard() {
  ...
  const handleManualCheck = async () => {
    try {
      ...
      alert(`Status check completed! Processed ${result.processedCount} hackathons, updated ${result.updatedCount}.`);
    ...
    return (
      <Alert className="border-red-500 bg-red-50">
        <AlertDescription className="text-red-700">
          Failed to load status dashboard: {summaryError?.message || statsError?.message}
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Status Dashboard</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{statusSummary?.activeHackathonsCount || 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{statusSummary?.currentStatus || 'N/A'}</div>
            <p className="text-xs text-muted-foreground">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Current status breakdown across all hackathons</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        ...
        <CardContent>
          {statusSummary?.recentChanges && statusSummary.recentChanges.length > 0 ? (
            ...
            <p className="text-center text-gray-500 py-4">No recent status changes</p>
          )}
        </CardContent>
      </Card>
      ...
      {auditStats && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Activity breakdown by action type and trigger</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-2">
                  {Object.entries(auditStats.logsByAction).map(([action, count]) => (
              ...
              <div>
                ...
                <div className="space-y-2">
                  {Object.entries(auditStats.logsByTrigger).map(([trigger, count]) => (
