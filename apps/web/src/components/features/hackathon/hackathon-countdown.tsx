...
export function HackathonCountdown({ 
  ...
  const isUrgent = timeData.timeRemaining !== null && timeData.timeRemaining < 24 * 60 * 60 * 1000; // Less than 24 hours
  ...
  if (compact) {
    return (
      <div className={cn(
        ...
        isPastDeadline ? 'text-red-600' : isUrgent ? 'text-orange-600' : 'text-gray-600',
        className
      )}>
        {showIcon && (
          isPastDeadline ? 
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        'text-lg font-bold',
        isPastDeadline ? 'text-red-600' : isUrgent ? 'text-orange-600' : 'text-green-600'
      )}>
        {timeData.timeRemaining !== null ? 
          formatTimeRemaining(timeData.timeRemaining) : 
          'Deadline passed'
