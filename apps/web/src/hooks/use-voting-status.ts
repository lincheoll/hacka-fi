...
export function useVotingStatus(hackathonId: string) {
  ...
  const totalParticipantsWithSubmissions = votingResults?.participants?.filter(
    p => p.submissionUrl
  ).length || 0;
  
  const votedCount = Object.keys(myVotes).length;
  const votingProgress = totalParticipantsWithSubmissions > 0 
    ? (votedCount / totalParticipantsWithSubmissions) * 100 
    : 0;

  const hasVoted = votedCount > 0;
  const hasCompletedVoting = votedCount === totalParticipantsWithSubmissions && totalParticipantsWithSubmissions > 0;
