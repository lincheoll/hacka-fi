...
export class RankingService {
  ...
  calculateRankings(
    ...
    this.logger.log(
      `Calculating rankings for ${participants.length} participants with ${totalJudges} judges`,
    );
    ...
    const baseResults = eligibleParticipants.map((participant) => {
      ...
      const scoreVariance =
        voteCount > 1
          ? votes.reduce(
              (sum, vote) => sum + Math.pow(vote.score - simpleAverage, 2),
              0,
            ) / voteCount
          : 0;
        ...
        baseResults.forEach((result) => {
          if (scoreRange > 0) {
            result.normalizedScore =
              ((result.weightedScore - minScore) / scoreRange) * 10;
          } else {
    ...
    const rankedResults: RankingResult[] = sortedResults.map(
      (result, index) => {
        let rank = index + 1;
        ...
        if (index > 0) {
          ...
          if (
            Math.abs(result.weightedScore - prevResult.weightedScore) < 0.01
          ) {
        ...
        return {
          ...
          normalizedScore:
            Math.round((result.normalizedScore || result.weightedScore) * 100) /
            100,
          rank,
          rankTier,
          scoreBreakdown: {
            ...
            normalized:
              Math.round(
                (result.normalizedScore || result.weightedScore) * 100,
              ) / 100,
    ...
    this.logger.log(
      `Rankings calculated: ${rankedResults.length} ranked participants`,
    );
  ...
  calculatePrizeDistribution(
    ...
    const topRanks = [
      ...new Set(rankings.slice(0, 3).map((r) => r.rank)),
    ].sort();

    if (topRanks.length >= 1) {
      const firstPlaceWinners = rankings.filter((r) => r.rank === topRanks[0]);
      const firstPrize =
        (totalPrizePool * distribution.first) / firstPlaceWinners.length;
    ...
    if (topRanks.length >= 2) {
      ...
      const secondPrize =
        (totalPrizePool * distribution.second) / secondPlaceWinners.length;
    ...
    if (topRanks.length >= 3) {
      ...
      const thirdPrize =
        (totalPrizePool * distribution.third) / thirdPlaceWinners.length;
